...
def generar_reporte_excel_cuatro_sumatorias_sin_pandas(archivo_excel, archivo_salida="reporte_parejas_turnos_cuatro_sumatorias.xlsx"):
    """
    Genera un reporte Excel con formato profesional del conteo de parejas de turnos,
    incluyendo cuatro columnas de sumatoria:
    ...
    try:
        ...
        crear_excel_formateado_cuatro_sumatorias_sin_pandas(datos_trabajadores, parejas_ordenadas, archivo_salida, 
        ...
        # Calcular y mostrar resumen de las cuatro sumatorias
        print(f"\nResumen de las cuatro sumatorias:")
...
def crear_excel_formateado_cuatro_sumatorias_sin_pandas(datos_trabajadores, parejas_ordenadas, archivo_salida, 
                                                        parejas_sumatoria1, parejas_sumatoria2, parejas_sumatoria3, parejas_resto):
    """Crea un archivo Excel con formato profesional incluyendo cuatro columnas de sumatoria"""
    ...
    # Escribir título
    ws['A1'] = "REPORTE DE PAREJAS DE TURNOS CON CUATRO SUMATORIAS"
    ...
    for row_idx, trabajador in enumerate(datos_trabajadores, 4):
        ...
        # Escribir sumatorias
        col_sumatoria1 = len(headers) - 2
        col_sumatoria2 = len(headers) - 1
        col_sumatoria3 = len(headers)
        col_sumatoria4 = len(headers) + 1
        ...
        # Centrar y resaltar sumatorias
        for col in [col_sumatoria1, col_sumatoria2, col_sumatoria3, col_sumatoria4]:
    ...
    for col in range(1, len(headers) + 1):
        if col >= len(headers) - 3:  # Columnas de sumatoria
    ...
    # Título del resumen
    ws_resumen['A1'] = "RESUMEN ESTADÍSTICO CON CUATRO SUMATORIAS"
    ...
    # Título
    ws_detalles['A1'] = "DETALLES POR TRABAJADOR CON CUATRO SUMATORIAS"
    ws_detalles['A1'].font = Font(bold=True, size=16, color="366092")
    ws_detalles.merge_cells('A1:G1')
...
def main():
    ...
    print("=== GENERADOR DE REPORTE EXCEL CON CUATRO SUMATORIAS ===\n")
    
    # Generar reporte
    datos_trabajadores = generar_reporte_excel_cuatro_sumatorias_sin_pandas(archivo)
    
    if datos_trabajadores is not None:
        print(f"\nEl archivo Excel se ha guardado como 'reporte_parejas_turnos_cuatro_sumatorias.xlsx'")
        print("El archivo contiene 3 hojas:")
        print("1. 'Reporte Parejas Turnos' - Tabla completa con cuatro columnas de sumatoria")
        print("2. 'Resumen' - Estadísticas generales incluyendo las cuatro sumatorias")
        print("3. 'Detalles por Trabajador' - Lista de parejas por trabajador con las cuatro sumatorias")
